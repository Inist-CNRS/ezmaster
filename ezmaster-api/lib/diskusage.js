...
module.exports = function(cb) {
  ...
  disk.check(cfg.dataInstancesPath, function(err, info) {
    ...
    diskUsage.diskApp.useDiskPercentage = parseFloat(
      ((info.total - info.free) * 100 / info.total).toFixed(0)
    );
    diskUsage.diskApp.fsIsAlmostFilled =
      diskUsage.diskApp.useDiskPercentage >= cfg.fullFsPercent;
    diskUsage.diskApp.maxFileCapSize = Math.floor(
      info.total * (cfg.fullFsPercent / 100) - (info.total - info.available)
    );
    ...
    disk.check("/etc/hosts", function(err, info2) {
      ...
      diskUsage.diskDocker.useDiskPercentage = parseFloat(
        ((info2.total - info2.free) * 100 / info2.total).toFixed(0)
      );
      diskUsage.diskDocker.fsIsAlmostFilled =
        diskUsage.diskDocker.useDiskPercentage >= cfg.fullFsPercent;
      diskUsage.diskDocker.maxFileCapSize = Math.floor(
        info2.total * (cfg.fullFsPercent / 100) -
          (info2.total - info2.available)
      );
      diskUsage.fsIsAlmostFilled =
        diskUsage.diskApp.fsIsAlmostFilled ||
        diskUsage.diskDocker.fsIsAlmostFilled;
