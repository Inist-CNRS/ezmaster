...
router.route("/").post(bodyParser(), function(req, res, next) {
  ...
  app.checkIfAppExistsInLocalCache(imageToPull, function(
    err,
    imgAlreadyPulled
  ) {
    if (err) {
        ...
        if (!app.dockerPullIsSpammed) {
          ...
          udisk(function(err, info) {
            ...
            if (
              ...
              socket.emit(
                "statusPull",
                event.status + ": " + event.progress.split("]")[1]
              );
  ...
  function afterTheImageIsPulled(err, output) {
    ...
    container.inspect(function(err, data) {
      ...
      if (err) {
        return res.status(500).send("Not enough space on the disk - " + err);
      }
      ...
      fs.writeFile(
        ...
        JSON.stringify(imageName, null, 2),
        function(err) {
          if (err) {
            return res.status(500).send("" + err);
          }
...
router.route("/:imageId").delete(function(req, res, next) {
  ...
  image.remove(function(err, datas, cont) {
    ...
    rimraf(cfg.dataApplicationsPath + "/" + nameManifest + ".json", function(
      err
    ) {
      if (err) {
        return next(err);
