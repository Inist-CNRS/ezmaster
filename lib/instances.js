...
module.exports.getInstances = function (cb) {
  ...
  async.parallel([
    ...
    results[1].forEach(function (dockerInstance) {
      results[0].forEach(function (manifest) {
        if (manifest.technicalName === dockerInstance.technicalName) {
          instances[manifest.technicalName] = _.assign(dockerInstance, manifest);
        }
      });
    });
    ...
    async.each(Object.keys(instances), (technicalName, cbDataPath) => {
      apps.readEzmasterAppConfig(instances[technicalName].app, (err, config) => {
...
/**
 * Returns an async function to get instanceId's manifest
 * @param instanceId {String} technical name of the instance
 * @return {Function}         f(cb), where cb(err, manifest)
 */
const getManifestFn = (instanceId) => {
  return function(cb) {
    const manifestPath = cfg.dataManifestsPath + '/' + instanceId + '.json';
    fs.readFile(manifestPath, 'utf8', (err, manifestContent) => {
      if (err) {
        debug(`Cannot read the ${manifestPath} file`, err);
        return cb(err);
      }

      const manifest = JSON.parse(manifestContent);
      manifest.technicalName = instanceId;
      cb(null, manifest);
    });
  };
};
...
/**
 * Returns an async function to get an instance's docker information
 *
 * @param container {Object}   container info {Id, Names, Image, State, Ports, ...}
 * @return {Function}          f(cb), where cb(err, instance)
 */
const getDockerInformationFn = (container) => {
  return function(cb) {
    const instance = {};

    // Example of data.Names[0] : /myprj-mystudy-5
    instance.technicalName = container.Names[0].split('/')[1];
    instance.containerId   = container.Id;
    instance.dataPath      = process.env.EZMASTER_PATH+'/data/instances/'
      +instance.technicalName+'/data/';
    instance.creationDate  = moment.unix(container.Created).format('YYYY/MM/DD HH:mm:ss');
    instance.app           = container.Image;
    ...
    if (container.State === 'running') {
      ...
      //   { IP: '0.0.0.0', PrivatePort: 3000, PublicPort: 32769, Type: 'tcp' } ]
      // we have to take the one having a PublicPort
      var portToHandle = container.Ports.filter(function (elt) {
        return elt.PublicPort !== undefined;
      });
      ...
      instance.target = container.Names[0].split('/')[1];
    ...
    // ezmaster.json
    apps.readEzmasterAppConfig(container.Image, (err, config) => {
      if (err || !config.dataPath) {
...
/**
 * Refactoring of module.exports.getInstances
 */
module.exports.getInstances0 = cb => {
  ...
  docker.listContainers({ all : true }, function (err, containers) {
    ...
    containers.forEach(container => {
      ...
      // Example of data.Names[0] : /myprj-mystudy-5
      const instanceId = container.Names[0].split('/')[1];
      functions.push(getManifestFn(instanceId));
      functions.push(getDockerInformationFn(container));
    });

    async.parallel(functions, (err, result) => {
      if (err) { return cb(err); }
      const instances = {};
      // TODO: recoller les morceaux
